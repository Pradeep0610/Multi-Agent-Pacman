...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for foodPos in foodList:
            ...
            closestFood = min(closestFood, thisDist)
        if action == Directions.STOP:
            score -= 3

        # I took the reciprocal of distance to the closest food. This means that the
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth):
            ...
            # Set value to infinity because you need to get the maximum value.
            value = float("-inf")
            # Store the possible movements in the current state in the list.
            legalMoves = state.getLegalActions()
            # Execute the loop for every possible move in current state
            for action in legalMoves:
                # Get the maximum values among the minimizer values
                value = max(value, minimizer(state.generateSuccessor(0,action),depth,1))
            return value # Returns maximum value found.
        ...
        def minimizer(state, depth, agentIndex):
            ...
            # Check if agentIndex is total number of agents - 1 or the last ghost.
            if agentIndex == state.getNumAgents() - 1:
                for action in legalMoves:
                    value = min(value, maximizer(state.generateSuccessor(agentIndex,action),depth+1))
            else:
                # Run loop for every possible move, even if its not the last ghost.
                for action in legalMoves:
